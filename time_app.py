...
if current_week_key not in st.session_state.default_blocks:
    st.session_state.st.session_state.default_blocks[current_week_key] = _build_default_blocks_from_weekplan(current_week_key)
...
#     # 2) 자동(default blocks)
#     auto_items = st.session_state.default_blocks.get(d, [])
...
auto_items = st.session_state.default_blocks.get(sel_day, [])
auto_main = [x for x in auto_items if not x.startswith("배경:")]
auto_routine = [x for x in auto_items if x.startswith("배경:")]

...
weeks_meta, weeks_map = month_weeks(datetime.date.today().year, datetime.date.today().month, week_start=0)
...
if uploaded_file:
    try:
        ...
        year = datetime.date.today().year
        month_num = MONTH_MAP.get(selected_month, datetime.date.today().month)
...
with st.expander("📋 오늘 체크 내역 보기/내보내기", expanded=False):
    rows = [{"날짜": today.isoformat(), "유형": kind, "할 일": text, "완료": (f"{kind} {text}" in completed)} for kind, text in tasks]
    ...
    st.download_button(
        "📥 오늘 체크 내역 CSV 다운로드",
        data=csv_today,
        file_name=f"today_tasks_{selected_week_key}_{today.isoformat()}.csv",
        mime="text/csv",
